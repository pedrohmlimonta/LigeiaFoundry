...
export class LigeiaCharacterSheet extends HandlebarsApplicationMixin(ActorSheetV2) {
  ...
  async _prepareContext(options) {
    ...
    // IMPORTANTE: calcular também para NPCs — os templates envolvem cada
    // item em {{#with (lookup ...)}} e mapas vazios fariam os itens
    // sumirem da lista. Para NPCs a ficha funciona igual à de player;
    // apenas a UI de XP (painel, campo e etiquetas) fica oculta.
  ...
  static async #rollWithDialog({ actor, label, attribute, improvement, bonus, reroll, crit }) {
    ...
    if (defAttr) {
      ...
      if (defActor) {
        ...
        let dBonus = 0;
        let dBaseDice = 2;
        if (shouldPromptRoll(defActor)) {
          const dLabel = CONFIG.LIGEIA?.defenseAttrs?.[defAttr] || defAttr;
          const dcfg = await requestRollConfig(defActor, {
            title: `${defActor.name} — Defesa (${dLabel})`,
            improvement: dImpBase,
            hint: `Rolagem de ${actor.name}. Role o máximo que puder.`,
            showDifficulty: false,
            allowDefense: false,
          });
          if (dcfg) {
            dBonus = dcfg.bonus || 0;
            if (dcfg.improvement != null) dImp = dcfg.improvement;
            dBaseDice = dcfg.baseDice ?? 2;
          }
        }
        defResult = await rollLigeia({
          ...
          improvement: dImp,
          baseDice: dBaseDice,
          bonus: (drm.all?.bonus || 0) + (drm.defense?.bonus || 0) + (d.rollBonus || 0) + dBonus,
